...
import { Db2MetaQuery } from './meta_query';
import { QueryCtrl } from 'grafana/app/plugins/sdk';
import { SqlPart, createPart } from './sql_part';
import { PanelEvents, QueryResultMeta } from '@grafana/data';
import { getAppEvents, TemplateSrv } from '@grafana/runtime';
import Db2QueryModel from 'db2_query_model';
import { ShowConfirmModalEvent, VariableWithMultiSupport } from 'types';
...
export class Db2QueryCtrl extends QueryCtrl {
  static templateUrl = 'partials/query.editor.html';

  formats: any[];
  queryModel: Db2QueryModel;
  metaBuilder: Db2MetaQuery;
  ...
  constructor($scope: any, $injector: any, private templateSrv: TemplateSrv, private uiSegmentSrv: any) {
    ...
    this.queryModel = new Db2QueryModel(this.target, templateSrv, this.panel.scopedVars);
    this.metaBuilder = new Db2MetaQuery(this.target, this.queryModel);
  ...
  updateProjection() {
    this.selectParts = map(this.target.select, (parts: any) => {
      return map(parts, createPart).filter((n) => n);
    });
    this.whereParts = map(this.target.where, createPart).filter((n) => n);
    this.groupParts = map(this.target.group, createPart).filter((n) => n);
  }
      ...
      .then((result: any) => {
        if (result.length === 1) {
          ...
          if (this.queryModel.hasUnixEpochTimecolumn()) {
            partModel = createPart({ type: 'macro', name: '$__unixEpochFilter', params: [] });
          } else {
            partModel = createPart({ type: 'macro', name: '$__timeFilter', params: [] });
          }
  ...
  addSelectPart(selectParts: any[], item: { value: any }, subItem: { type: any; value: any }) {
    ...
    let partModel = createPart({ type: partType });
    if (subItem) {
    ...
    if (addAlias) {
      ...
      partModel = createPart({ type: 'alias', params: [selectParts[0].params[0].replace(/"/g, '')] });
  ...
  addGroup(partType: string, value: string) {
    ...
    const partModel = createPart({ type: partType, params: params });
    ...
    for (const selectParts of this.selectParts) {
      if (!selectParts.some((part) => part.def.type === 'aggregate')) {
        const aggregate = createPart({ type: 'aggregate', params: ['avg'] });
        selectParts.splice(1, 0, aggregate);
        if (!selectParts.some((part) => part.def.type === 'alias')) {
          const alias = createPart({ type: 'alias', params: [selectParts[0].part.params[0]] });
  ...
  addWhereAction(part: any, index: any) {
    switch (this.whereAdd.type) {
      case 'macro': {
        const partModel = createPart({ type: 'macro', name: this.whereAdd.value, params: [] });
        if (this.whereParts.length >= 1 && this.whereParts[0].def.type === 'macro') {
          // replace current macro
          this.whereParts[0] = partModel;
        } else {
      ...
      default: {
        this.whereParts.push(createPart({ type: 'expression', params: ['value', '=', 'value'] }));
      }
    }
